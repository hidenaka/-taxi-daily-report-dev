...
// 指定中心から radius_m の範囲で、自動車道路 (highway) を全て取得。歩道・階段等は除外。
// 戻り値: [{ id, geometry:[{lat,lng}...], tags }]
export async function fetchAllRoadsAround(lat, lng, radius_m = 400) {
  const exclude = 'footway|path|cycleway|pedestrian|steps|track|bridleway';
  const q = `
    [out:json][timeout:25];
    (
      way["highway"]["highway"!~"^(${exclude})$"](around:${radius_m},${lat},${lng});
    );
    out geom;
  `;
