...
（`payrollMode`）に関わらず歩率テーブルの入力が必須・常時表示に
なっている。固定歩率の会社は乗務数別の歩率表を持たず、固定率（`fixedRate`）だけで
...
- 段階3 dev 反映後、ユーザーが実機確認し「OKだが歩率テーブルがない会社もある」と
  指摘（2026-05-19）。
- タクシー会社の給与体系は「変動歩率（乗務数別の段階歩率）」「固定歩率（売上×固定
  歩率）」「月給制（売上非依存）」の3種。
...
- 月給制は `payroll.js` に存在せず、新モード追加＋ホーム画面（売上→歩率→着地
  ...
  （2026-05-19 ユーザー判断）。本設計のスコープは変動歩率＋固定歩率のみ。

## スコープ

含む:
- 会社管理UIの給与モードラベルを「変動歩率／固定歩率」に変更
...
常時表示（固定歩率の会社に無意味な歩率テーブルが残り、指摘に応えない）／モード別
の別フォーム（オーバー）。
...
1. **給与モードラベル変更**: `<select id="companyPayrollMode">` の option 表示
   ...
   - `step_rate`: 「段階歩率（step_rate）」→「変動歩率」
   - `fixed_rate`: 「固定率（fixed_rate）」→「固定歩率」
...
1. **会社読み込み**（既存会社選択）: `fillCompanyForm(c, false)` が各フィールドを
   ...
   `applyCompanyPayrollModeUI()` で表示を体系に合わせる。固定歩率の会社は
...
2. **新規会社**: `newCompanyTemplate()` は従来通り `step_rate` 既定 →
   `applyCompanyPayrollModeUI()` で歩率テーブル表示。ユーザーが「固定歩率」に
...
3. **保存**: `saveCompanyBtn` の onclick は従来通り `form` に `rateTable`
   ...
   に渡す。`buildCompanyDoc` が `payrollMode` で取捨選択。固定歩率時の歩率
   テーブルは `display:none` でもDOMに残るため `collectRateTable` は値を返すが、
   `buildCompanyDoc` が破棄するため無害。

### エラーハンドリング

- 固定歩率で `fixedRate` 未入力／非数値 → `buildCompanyDoc` が
  `{ error: '...' }` を返し、保存ハンドラが赤字表示。
- 変動歩率で `rateTable` 不正 → 同様にエラー。
...
（変動歩率 doc は `fixedRate` を持たず、固定歩率 doc は `rateTable` を持たない）
ため差し替える:
